...
- AC-005 [MUST]: **Given** the hub after P3, **When** it is built and tested, **Then** `npm run build`, `tsc` and the full suite pass. `grep -rni financ src` matches no code that imports or queries financial modules or tables. The only hits are the link card, comments explaining the removal, tests, and the unrelated email-topic taxonomy in `seed.ts`. This is extraction AC-006. Risk: MED
